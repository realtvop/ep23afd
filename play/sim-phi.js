...
const showBrowser = parser.getBrowser().name;
var pausemenu;
var nowShowing = "play"; // 当前展示页面 songSelect/play
/* var bottomStat = {
	appearTimestrap: -250,
	noteNum: -1, // note编号
	status: "null", // 0:perfect,1:good,2:bad,3:miss
	time: 0, // 晚的时间,如晚11.4ms打击为"+11.4",早11.4ms为"-11.4"
	isFlick: false,
	update: (i, time, showTime = false) => {
		bottomStat.appearTimestrap = Date.now();
		bottomStat.noteNum = Renderer.notes.indexOf(i);
		bottomStat.status = getnotestatus(i.status);
		bottomStat.time = time.toFixed(0);
		bottomStat.isFlick = showTime;
	},
}; */
var songSelectValue = {
...
document.body.style.backdropFilter = `blur(50px) brightness(${selectglobalalpha.value})`;
document.body.style.webkitBackdropFilter = `blur(50px) brightness(${selectglobalalpha.value})`;

/* function getnotestatus(status) {
	switch (status) {
		case 1: return "PERFECT(LATE)";
		case 2: return "BAD";
		case 3: return "GOOD(LATE)";
		case 4: return "PERFECT";
		case 5: return "PERFECT(EARLY)";
		case 6: return "BAD";
		case 7: return "GOOD(EARLY)";
	}
} */
...
function pausedMenu() {
	return mdui.dialog({
		title: 'Paused/已暂停',
		content: 'Tip:如果界面卡顿(帧数极低)/音画严重不同步,请尝试重新整理页面.',
		buttons: [
			{
				text: 'Settings/设定',
	...
	judgeNote(notes, realTime, width) {
		for (const i of notes) {
			...
			if ((i.realTime - realTime < -(hyperMode.checked ? 0.12 : 0.16) && i.frameCount > (hyperMode.checked ? 3 : 4)) && !i.status2) {
				...
				// bottomStat.update(i, i.realTime - realTime);
				i.scored = true;
			} else if (i.type == 1) {
				for (let j = 0; j < this.length; j++) {
					if (this[j].type == 1 && this[j].isInArea(i.offsetX, i.offsetY, i.cosr, i.sinr, width) && i.realTime - realTime < 0.2 && (i.realTime - realTime > -(hyperMode.checked ? 0.12 : 0.16) || i.frameCount < (hyperMode.checked ? 3 : 4))) {
						if (i.realTime - realTime > 0.16 - judgeOffset) {
							...
							if ($("hitSong").checked) playSound(res["HitSong0"], false, true, 0, localStorage.getItem("input-hitSongVolume"));
							clickEvents1.push(ClickEvent1.getClickGoodLate(i.projectX, i.projectY));
						}
						if (i.status) {
				...
				if (i.status == 4 && i.realTime - realTime < 0) {
					...
					clickEvents1.push(this.isKeyboard ? ClickEvent1.getClickKeyboard(i.projectX, i.projectY) : ClickEvent1.getClickPerfect(i.projectX, i.projectY));
					stat.addCombo(4, 2);
					// bottomStat.update(i, i.realTime - realTime);
					i.scored = true;
				} else if (!i.status) {
...
class ClickEvent1 {
	...
	static getClickPerfect(offsetX, offsetY) {
		// if (isAutoplay) return this.getClickAutoplay(offsetX, offsetY);
	...
	static getClickGoodLate(offsetX, offsetY) {
		// if (isAutoplay) return this.getClickAutoplay(offsetX, offsetY);
		return new ClickEvent1(offsetX, offsetY, "rgba(180,255,225,0.9215686)", 3, "#b4ffe1");
	}
	static getClickKeyboard(offsetX, offsetY) {
		// if (isAutoplay) return this.getClickAutoplay(offsetX, offsetY);
		return new ClickEvent1(offsetX, offsetY, "rgba(255,180,180,0.9215686)", 3, "#ffb4b4");
	}
	static getClickAutoplay(offsetX, offsetY) {
		return new ClickEvent1(offsetX, offsetY, "rgba(255,255,255,0.9215686)", 3, "#fff");
	}
}
//适配PC鼠标
const isMouseDown = {};
...
// 获取实时音量 by ChatGPT
const bgmAnalyser = actx.createAnalyser();
bgmAnalyser.fftSize = 128; // 设置FFT大小
const bufferLength = bgmAnalyser.frequencyBinCount;
const dataArray = new Uint8Array(bufferLength);
const playSound = (res, loop, isOut, offset, volume = 1, isBgm=false) => {
	...
	if (isBgm) {
		bufferSource.connect(bgmAnalyser);
		// bgmAnalyser.connect(actx.destination);
	}
...
function getVolume() {
	bgmAnalyser.getByteFrequencyData(dataArray);
	const total = dataArray.reduce((acc, val) => acc + val, 0);
	const average = total / bufferLength;
	const volume = average / 255; // 将平均值转换为0-1之间的值
...
window.onload = function () {
	...
	(async function () {
		...
		res["Clicks"]["rgba(180,255,225,0.9215686)"] = await qwqImage(res["clickRaw"], "rgba(180,255,225,0.9215686)"); //#9ef3d5
		res["Clicks"]["rgba(162,255,204,0.9215686)"] = await qwqImage(res["clickRaw"], "rgba(162,255,204,0.9215686)"); //#a2ffcc
		res["Clicks"]["rgba(255,255,255,0.9215686)"] = await qwqImage(res["clickRaw"], "rgba(255,255,255,0.9215686)"); //#fff
...
const stat = {
	...
	get scoreNum() {
		var a;
		if (this.combos[0]<240)
			a = 1e6 * (this.perfect * 0.9 + this.good * 0.585 + this.maxcombo * 0.1) / 305;
		else if (timeChart <= 86.654)
			a = ((1e6 * (this.perfect * 0.9 + this.good * 0.585 + this.maxcombo * 0.1) / 305) - (1e6 * (this.perfect * 0.9 + this.good * 0.585 + this.maxcombo * 0.1) / 2500)) / 0.5 * (timeChart - 86.154)
			else
				a = 1e6 * (this.perfect * 0.9 + this.good * 0.585 + this.maxcombo * 0.1) / 2500;
...
function playBgm(data, offset) {
	...
	stopPlaying.push(playSound(data, false, true, offset, localStorage.getItem("input-songVolume"), true));
}
let fucktemp = false;
let fucktemp2 = false;
//作图
function loop() {
	...
	ctx.drawImage(Renderer.bgImageBlur, ...adjustSize(Renderer.bgImageBlur, canvas, 1.1)); //对背景图片进行模糊处理，注释部分为选择
	...
	ctx.textAlign = "center";
	ctx.textBaseline = "middle";
	ctx.fillText(`Elecphi ${stat.combos[0] < 240 ? "v0.0.14" : "2023 April Fools' Day Special Version"} by 飞机上的长电视 with lchzh3473/sim-phi - ${stat.combos[0] < 240 ? showPlatform :`${showBrowser} on Phi\\g/os`}`, canvas.width / 2, y);
	/* ctx.textAlign = "left";
	if (showInfo.checked) {
...
function qwqdraw1(now) {
	...
	ctxos.globalAlpha = 1;
	ctxos.fillStyle = "rgba(255, 255, 255, 0)";
	ctxos.resetTransform();
	...
	if (stat.combos[0] < 240)
		ctxos.drawImage(res["ProgressBar"], timeChart / 115 * 1920 - 1920, 0);
	else
		ctxos.drawImage(res["ProgressBar"], Math.abs(getVolume() - 0.3) / 2 * 1920 - 1920, 0);
	//绘制文字
	ctxos.resetTransform();
	ctxos.fillStyle = "#fff";
	//开头过渡动画
	...
	if (stat.combo > 2/*  && !isAutoplay */) {
		...
		if (timeChart > 166.5 && timeChart < 183.6) {
			if (timeChart < 167.75) {
				ctxos.fillText("(⊙o⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1070].realTime - 0.16) {
				ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1070].realTime + 0.5) {
				switch (Renderer.tapholds[1070].status) {
					case 1:{
						ctxos.fillText("O(≧▽≦)O", wlen, lineScale * 1.375);
						break;
					};
					case 4:{
						ctxos.fillText("O(≧▽≦)O", wlen, lineScale * 1.375);
					...
					case undefined:{
						if (timeChart > Renderer.tapholds[1070].realTime + 0.16)
							ctxos.fillText("?", wlen, lineScale * 1.375);
						else
							ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
						break;
					};
				}
			} else /* 2 */ if (timeChart < 172) {
				ctxos.fillText("(⊙o⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1071].realTime - 0.16) {
				ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1071].realTime + 0.5) {
				switch (Renderer.tapholds[1071].status) {
					case 1:{
						ctxos.fillText("O(≧▽≦)O", wlen, lineScale * 1.375);
						break;
					};
					...
					case undefined:{
						if (timeChart > Renderer.tapholds[1071].realTime + 160)
							ctxos.fillText("?", wlen, lineScale * 1.375);
						else
							ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
						break;
			...
			} else /* 3 */ if (timeChart < 176.25) {
				ctxos.fillText("(⊙o⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1072].realTime - 0.16) {
				ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1072].realTime + 0.5) {
				switch (Renderer.tapholds[1072].status) {
					case 1:{
						ctxos.fillText("O(≧▽≦)O", wlen, lineScale * 1.375);
						break;
					};
					...
					case undefined:{
						if (timeChart > Renderer.tapholds[1072].realTime + 160)
							ctxos.fillText("?", wlen, lineScale * 1.375);
						else
							ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
						break;
			...
			} else /* 4 */ if (timeChart < 180) {
				ctxos.fillText("(⊙o⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1073].realTime - 0.16) {
				ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
			} else if (timeChart < Renderer.tapholds[1073].realTime + 0.5) {
				switch (Renderer.tapholds[1073].status) {
					case 1:{
						ctxos.fillText("O(≧▽≦)O", wlen, lineScale * 1.375);
						break;
					};
					...
					case undefined:{
						if (timeChart > Renderer.tapholds[1073].realTime + 160)
							ctxos.fillText("?", wlen, lineScale * 1.375);
						else
							ctxos.fillText("(⊙ω⊙)", wlen, lineScale * 1.375);
						break;
	...
	ctxos.textAlign = "right";
	ctxos.fillText(stat.combos[0] < 240 ? "lN Lv.I2" : "SP Lv.?", canvasos.width - lineScale * 0.5, canvasos.height - lineScale * 0.66);
	...
	// //notes总数
	// ctxos.fillStyle = comboColor[0];
	// ctxos.fillText(stat.combos[0], lineScale * 0.25, canvasos.height - lineScale * 0.1);
	// //perfect
	// ctxos.fillStyle = comboColor[2];
	// ctxos.fillText(stat.perfect, lineScale * 1.1 + lineScale * 0.25, canvasos.height - lineScale * 0.1);
	// //good
	// ctxos.fillStyle = comboColor[1];
	// ctxos.fillText(stat.good, lineScale * 2 * 1.1 + lineScale * 0.25, canvasos.height - lineScale * 0.1);
	// //bad
	// ctxos.fillStyle = comboColor[3];
	// ctxos.fillText(stat.bad, lineScale * 3 * 1.1 + lineScale * 0.25, canvasos.height - lineScale * 0.1);
	// //miss
	// ctxos.fillStyle = comboColor[4];
	// ctxos.fillText(stat.noteRank[2], lineScale * 4 * 1.1 + lineScale * 0.25, canvasos.height - lineScale * 0.1);
	...
	// if (stat.discombo >= 2) {
	// 	ctxos.font = `${lineScale * 0.3}px Mina`;
	// 	ctxos.fillText("DISCOMBO", canvasos.width / 3, canvasos.height - lineScale * 0.34);
	// 	ctxos.font = `${lineScale * 0.65}px Mina`;
	// 	ctxos.fillText(stat.discombo, canvasos.width / 3, canvasos.height - lineScale * 0.66);
	// }
	//底部显示信息
	/* const nowBtmStat = bottomStat;
	const nowTimestrap = Date.now();
	if(nowBtmStat.appearTimestrap + 300 >= nowTimestrap) {
		ctxos.font = `${lineScale * 0.3}px Mina`;
		ctxos.fillText(nowBtmStat.noteNum, canvasos.width / 2, canvasos.height - lineScale * 0.34);
		// ctxos.globalAlpha = (nowTimestrap - nowBtmStat.appearTimestrap <= 50 ? nowTimestrap - nowBtmStat.appearTimestrap : 50) / 50
		ctxos.font = `${lineScale * 0.6}px Mina`;
		ctxos.fillText(
			nowBtmStat.status + (nowBtmStat.status !== 3 ? `  ${nowBtmStat.time < 0 ? "-" : "+"}${Math.abs(nowBtmStat.time)}ms` : ""),
			canvasos.width / 2, canvasos.height - lineScale * 0.66);
	} */
...
function qwqdraw2() {
	...
	xhr.onload = async () => {
		...
		const timeout = setTimeout(() => {
			if (!fucktemp) return;
			stopPlaying.push(playSound(bgm, true, true, 0, localStorage.getItem("input-songVolume"), true));
			qwqEnd.reset();
			qwqEnd.play();
